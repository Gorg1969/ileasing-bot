...
Хранит только listings (спарсенные карточки) + base64 изображения (до 3 штук).
"""
    ...
    def _migrate(self, conn):
        ...
        try:
            ...
            if "images_base64" not in cols:
                logger.info("🔧 Добавляю колонку images_base64...")
                conn.execute("ALTER TABLE listings ADD COLUMN images_base64 TEXT")
                conn.commit()
                logger.info("✅ Колонка images_base64 добавлена")
            ...
            try:
                conn.execute("""
                    INSERT INTO listings (
                        ...
                        image, images_path, image_base64, images_base64,
                        category, status
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending')
                """, (
                    external_id, url, title, price, price_value, leasing,
